...
try:
    import pytest
except ImportError:  # pragma: no cover - only used when unittest imports this file.
    pytest = None


if pytest is not None:
    pytestmark = pytest.mark.skipif(
        os.getenv("HANDWRITING_RUN_LOAD_TEST") != "1",
        reason="backend load test is opt-in; set HANDWRITING_RUN_LOAD_TEST=1 with a running backend",
    )
else:
    pytestmark = ()
...
def test_generate_handwriting_concurrency():
    if pytest is None:
        raise unittest.SkipTest("pytest is not installed; load test is pytest-only")
    ...
    if total_requests < 4:
        fail_test("HANDWRITING_LOAD_REQUESTS should be >= 4 to cover the 3+ requests scenario.")
    ...
    try:
        ...
        fail_test(f"Failed to connect backend at {base_url}: {e}")

    if status_code != 200 or not isinstance(fonts, list) or len(fonts) == 0:
        fail_test(f"/api/fonts_info failed: status={status_code}, body={fonts}")
    font_option = fonts[0]
