...
async def generate_handwritten_document(
    request: Request,
    file: UploadFile = File(...),
    output_format: str = Form("pdf"),
    font_name: str = Form(DEFAULT_FONT_NAME),
):
    """Generate an editable-style handwritten document without embedded images.

    This path is intended for math homework documents: text runs are rendered
    with the handwriting font, formulas are kept as Word Office Math through the
    Markdown -> DOCX conversion, and the final PDF/DOCX is rejected if images or
    LaTeX residue remain.
    """

    if file is None or file.filename == "":
        return JSONResponse({"error": "No file part in the request"}, status_code=400)
    suffix = Path(file.filename).suffix.lower()
    if suffix not in {".md", ".markdown", ".docx", ".pdf"}:
        return JSONResponse(
            {"error": "Only .md, .docx and text-layer .pdf are supported by the no-image handwritten document pipeline"},
            status_code=400,
        )
    if output_format not in {"pdf", "docx"}:
        return JSONResponse({"error": "output_format must be pdf or docx"}, status_code=400)
    ...
    try:
        ...
        output_stem = f"{source_path.stem}_handwritten"
        ...
        result_path = Path(report[output_format])
        ...
        filename = f"{output_stem}.{output_format}"
        return Response(
            content=result_bytes,
            media_type=media_type,
            headers={
                "Content-Disposition": f"attachment; filename={filename}",
                "X-Handwriting-Document-Report": json.dumps(
                    {
                        "docx": report["docx_inspect"],
                        "pdf": report["pdf_inspect"],
                        "removed_images": report["normalize"]["removed_images"],
                    },
                    ensure_ascii=False,
                ),
            },
        ...
        logger.exception("No-image handwritten document generation failed")
        return JSONResponse({"status": "error", "message": str(e)}, status_code=500)
    finally:
        safe_remove_directory(temp_dir)
